<?php
    ...
    while(count($array) > $i){
        if(@$_SESSION["log_id"] == ''){
            ...
            join mutacao on ave.ave_mutacao = mutacao.mut_id where (ave_estado = 1 or ave_estado = 4) and (especie.esp_nome LIKE '%$array[$i]%' or variedade.var_nome LIKE '%$array[$i]%' or variedade.var_cientifico LIKE '%$array[$i]%' or mutacao.mut_nome LIKE '%$array[$i]%')");
            ...
            $aves_venda = mysqli_query($conn, "Select * from ave inner join especie on especie.esp_id = ave.ave_especie 
            join variedade on ave.ave_variedade = variedade.var_id
            join mutacao on ave.ave_mutacao = mutacao.mut_id where (ave_estado = 1 or ave_estado = 4 and ave_estado = 2) and ave_log_id != '$_SESSION[log_id]' and (especie.esp_nome LIKE '%$array[$i]%' or variedade.var_nome LIKE '%$array[$i]%' or variedade.var_cientifico LIKE '%$array[$i]%' or mutacao.mut_nome LIKE '%$array[$i]%')");
            if(mysqli_num_rows($aves_venda) > 0){
                while($ave_v = mysqli_fetch_array($aves_venda)){
                    echo '<div class="col-sm-12 col-md-6 col-xl-4 col-xxl-3" style="margin-bottom: 1%;">
                        <div class="card">
