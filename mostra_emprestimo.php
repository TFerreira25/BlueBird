<?php
    ...
    while(count($array) > $i){
        ...
        join mutacao on ave.ave_mutacao = mutacao.mut_id where ave_estado = 2 and ave_log_id != '$_SESSION[log_id]' and (especie.esp_nome LIKE '%$array[$i]%' or variedade.var_nome LIKE '%$array[$i]%' or variedade.var_cientifico LIKE '%$array[$i]%' or mutacao.mut_nome LIKE '%$array[$i]%')");
